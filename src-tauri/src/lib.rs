...
struct AnalysisResult {
    ...
    /// LLM coaching advice generated by Ollama (None if Ollama is unavailable)
    coaching_text: Option<String>,
}

// Type alias for the engine state managed by Tauri
type EngineState = Mutex<StockfishEngine>;

/// Read-only knowledge base loaded once at startup and shared across calls.
pub struct KnowledgeState(pub Vec<knowledge::KnowledgeDoc>);
...
async fn analyze_position(
    ...
    knowledge: tauri::State<'_, KnowledgeState>,
    ...
    // Release the engine lock before the (potentially slow) Ollama call.
    drop(engine);
    ...
    // --- Retrieve relevant knowledge docs and call Ollama for coaching ---
    let coaching_text = if let Some(ref ct) = comparison_text {
        let relevant_docs = knowledge::retrieve_relevant(ct, &knowledge.0, 3);
        let prompt = llm::build_coaching_prompt(ct, &relevant_docs);
        match llm::call_ollama(prompt).await {
            Ok(text) => Some(text),
            Err(e) => {
                log::warn!("Ollama coaching unavailable: {}", e);
                None
            }
        }
    } else {
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            app.manage(KnowledgeState(knowledge::load_knowledge()));
